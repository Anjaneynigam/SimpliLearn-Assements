
import java.io.File;
import java.io.IOException;
import java.util.Arrays;
import java.util.Scanner;
public class LockedMe {
       static String DIRECTORY;
       File folder_name;
       public LockedMe() {
         DIRECTORY = System.getProperty("user.dir");
         folder_name = new File(DIRECTORY+"/files");
         if (!folder_name.exists())
         folder_name.mkdirs();
 System.out.println("DIRECTORY : " + folder_name.getAbsolutePath());
 }
 private static final String WELCOME_SCREEN =
		 "\n==========================================="+ 
           "\n @@      Welcome to LockedMe.com      @@ "+
          "\n==========================================="+ 
         "\n@@ Developed by: #FSD Anjaney Nigam @@@@\n";
 private static final String PRIMARY_MENU =
	"\n==========================================="+
            "\n  MAIN MENU "+
     "\n==========================================="+
             "\n Please choose an Option \n"+
       " OPTION 1 -> List files in directory\n"+
       " OPTION 2 -> Add, Delete or Search\n"+
       " OPTION 3 -> Exit Program";
 private static final String SECONDARY_MENU =
       " \nSelect any of the following: \n"+
          "    a -> Add a file\n"+
          "    b -> Delete a file\n"+
          "    c -> Search a file\n"+
          "    x -> GoBack";
void showPrimaryMenu() {
System.out.println(PRIMARY_MENU);
try{
Scanner sc = new Scanner(System.in);
int option = sc.nextInt();
switch (option){
case 1 : {
showFiles();
showPrimaryMenu();
}
case 2 : {
showSecondaryMenu();
}
case 3 : {
System.out.println("Thank You");
System.exit(0);
}
default: showPrimaryMenu();
}
}
catch (Exception e){
System.out.println("Please choose an Option 1, 2 or 3");
//System.out.println(e.getMessage());
showPrimaryMenu();
}
}
void showSecondaryMenu() {
System.out.println(SECONDARY_MENU);
try{
Scanner sc = new Scanner(System.in);
char[] input = sc.nextLine().toLowerCase().trim().toCharArray();
char option = input[0];

switch (option){
case 'a' : {
System.out.println("Adding a file...Please Enter a File Name:");

String filename = sc.next().trim().toLowerCase();
addFile(filename);
break;
}
case 'b' : {
System.out.println("Deleting a file...Please Enter a File Name :");

String filename = sc.next().trim();
deleteFile(filename);
break;
}
case 'c' : {
System.out.println("Searching a file...Please Enter a File Name :");

String filename = sc.next().trim();
searchFile(filename);
break;
}
case 'x' : {
System.out.println("Go Back to MAIN menu");
showPrimaryMenu();
break;
}
default : System.out.println("Please choose an Option a, b, c or x");
}
showSecondaryMenu();
}
catch (Exception e){
System.out.println("Please choose an Option a, b, c or x");
showSecondaryMenu();
}
}
void showFiles() {
if (folder_name.list().length==0)
System.out.println("The folder is empty");
else {
String[] list = folder_name.list();
System.out.println("The files in"+ folder_name + " are :");
Arrays.sort(list);
for (String str:list) {
System.out.println(str);
}
}
}
// Add File
void addFile(String filename) throws IOException {
File filepath = new File(folder_name +"/"+filename);
String[] list = folder_name.list();
boolean value = filepath.createNewFile();
if(value) {
	System.out.println("File " +filename+ " added to folder/directory ");
}else {
	System.out.println("The File already exists in Folder/Directory");
}
}
 // Delete File
void deleteFile(String filename) {
File filepath = new File(folder_name +"/"+filename);
boolean value = filepath.delete();
if(value) {
	System.out.println("File " + filename + " deleted from folder/directory");
}else {
	System.out.println("This file is not in list: Delete Operation faild!");
}	
}
//Search File
void searchFile(String filename) {
String[] list = folder_name.list();
for (String file: list) {
if (filename.equals(file)) {
System.out.println("FOUND : File" + filename + " exists at " + folder_name);
return;
}
}
System.out.println("File NOT found (FNF)");
}
public static void main(String[] args) {
System.out.println(WELCOME_SCREEN);
LockedMe menu = new LockedMe();
menu.showPrimaryMenu();
}
}